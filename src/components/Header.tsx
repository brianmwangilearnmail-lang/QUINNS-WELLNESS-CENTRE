        ...
        <div className="flex justify-between items-center h-24">
          ...
          <nav className="hidden md:flex space-x-10">
            <button 
              ...
              className={`font-bold text-sm uppercase tracking-wider transition-colors ${currentPage === 'home' ? 'text-[#14532d]' : 'text-gray-900 hover:text-[#14532d]'}`}
            >
              Products
            </button>
            <button 
              onClick={() => onPageChange('about')} 
              className={`font-bold text-sm uppercase tracking-wider transition-colors ${currentPage === 'about' ? 'text-[#14532d]' : 'text-gray-900 hover:text-[#14532d]'}`}
            >
              About Us
            </button>
            <button 
              onClick={() => onPageChange('contact')} 
              className={`font-bold text-sm uppercase tracking-wider transition-colors ${currentPage === 'contact' ? 'text-[#14532d]' : 'text-gray-900 hover:text-[#14532d]'}`}
            >
              Contact
            </button>
            <button 
              onClick={() => onPageChange('science')} 
              className={`font-bold text-sm uppercase tracking-wider transition-colors ${currentPage === 'science' ? 'text-[#14532d]' : 'text-gray-900 hover:text-[#14532d]'}`}
            >
              Our Science
            </button>
          </nav>
          ...
          <>
            ...
            >
              <div className="p-6 flex items-center justify-between border-b border-gray-100">
                <span className="font-display font-black text-2xl text-[#14532d] tracking-tighter">
                  QUINS WELLNESS
                </span>
              ...
              <div className="flex flex-col p-6 space-y-6 flex-grow overflow-y-auto">
                <button 
                  ...
                  className={`text-left font-black text-2xl tracking-tight transition-colors ${currentPage === 'home' ? 'text-[#14532d]' : 'text-gray-900'}`}
                >
                  SHOP PRODUCTS
                </button>
                <button 
                  onClick={() => handleMobileNav('about')} 
                  className={`text-left font-black text-2xl tracking-tight transition-colors ${currentPage === 'about' ? 'text-[#14532d]' : 'text-gray-900'}`}
                >
                  ABOUT US
                </button>
                <button 
                  onClick={() => handleMobileNav('science')} 
                  className={`text-left font-black text-2xl tracking-tight transition-colors ${currentPage === 'science' ? 'text-[#14532d]' : 'text-gray-900'}`}
                >
                  OUR SCIENCE
                </button>
                <button 
                  onClick={() => handleMobileNav('contact')} 
                  className={`text-left font-black text-2xl tracking-tight transition-colors ${currentPage === 'contact' ? 'text-[#14532d]' : 'text-gray-900'}`}
                >
                  CONTACT
                </button>
              </div>
